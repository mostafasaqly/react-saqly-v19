const e={id:6,title:"Effects و Lifecycle",level:"متوسط",lessons:["ما هو Side Effect؟","Hook useEffect","اعتماديّات Effect","دوال التنظيف","جلب البيانات بـ useEffect","أخطاء useEffect الشائعة","متى لا تحتاج useEffect"],intro:"حتى الآن Componentsنا ترسم واجهة من props و state فقط. لكن التطبيقات الحقيقية تتحدّث مع العالم الخارجي: مؤقّتات، وواجهات المتصفح، وخوادم. هذا ما تفعله Effects.",content:[{type:"heading",text:"1. ما هو Side Effect؟"},{type:"paragraph",text:"Side Effect هو أي شيء يفعله Component خارج React: ضبط مؤقّت، جلب بيانات، القراءة/الكتابة في localStorage، تغيير عنوان الصفحة. هذه لا تحدث أثناء الرسم — تحتاج مكاناً خاصاً يعمل بعد تحديث الشاشة، وهو useEffect."},{type:"heading",text:"2. Hook useEffect"},{type:"paragraph",text:"useEffect يشغّل كوداً بعد رسم Component. Middleware الثاني (المصفوفة) يتحكّم في موعد التشغيل."},{type:"code",code:`useEffect(() => {
  document.title = "Welcome!";
}, [/* الاعتماديّات */]);`},{type:"heading",text:"3. اعتماديّات Effect"},{type:"list",items:["بدون مصفوفة → بعد كل رسم","[] فارغة → مرة واحدة بعد أول رسم","[count] → بعد أول رسم وكلما تغيّر count"]},{type:"code",code:`useEffect(() => {
  console.log("mounted");
}, []); // مرة واحدة

useEffect(() => {
  document.title = \`Count: \${count}\`;
}, [count]); // عند تغيّر count`},{type:"heading",text:"4. دوال التنظيف"},{type:"paragraph",text:"بعض Effects تبدأ شيئاً يستمرّ (مؤقّت أو اشتراك). أرجِع دالة تنظيف لإيقافه قبل التشغيل التالي وعند إزالة Component."},{type:"code",code:`useEffect(() => {
  const id = setInterval(() => console.log("tick"), 1000);
  return () => clearInterval(id); // تنظيف
}, []);`},{type:"tip",text:"نظّف دائماً المؤقّتات والمستمعين والاشتراكات لمنع تسرّب الذاكرة."},{type:"heading",text:"5. جلب البيانات بـ useEffect"},{type:"paragraph",text:"الاستخدام الكلاسيكي: تحميل بيانات من خادم عند ظهور Component. تعامل دائماً مع الحالات الثلاث: تحميل، خطأ، بيانات."},{type:"code",code:`useEffect(() => {
  async function load() {
    try {
      setLoading(true);
