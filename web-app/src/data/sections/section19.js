...
export default {
  ...
  content: [
    ...
    {
      type: "paragraph",
      text: "هذا أعقد مشروع في الكورس. خطّط بعناية قبل البرمجة — افهم أين تعيش كل حالة ولماذا.",
    },
    {
      type: "list",
      items: [
        "تصفّح منتجات مجلوبة من API حقيقي (Fake Store API)",
        "إضافة منتجات للسلّة مع تتبّع الكمّيات",
        "تعليم المنتجات كمفضّلة (تبديل أيقونة القلب)",
        "إجمالي سلّة حيّ + حالات تحميل وخطأ يديرها Redux",
      ],
    },
    {
      type: "code",
...
└── components/
  ...
  reducers: { addToCart, removeFromCart, clearCart },
    ...
    {
      type: "paragraph",
      text: "إن كان المنتج موجوداً في السلّة، زِد كمّيته. وإلا أضف عنصراً جديداً بـ qty: 1. هذا أهم منطق في السلّة.",
    },
    {
      type: "code",
...
dispatch(removeFromCart(product.id));`,
    ...
    {
      type: "paragraph",
      text: "removeFromCart يفلتر العنصر بالـ id. الحمولة هي id المنتج (رقم)، وليست كائن المنتج كاملاً. أبقِ الحمولات صغيرة.",
    },
    {
      type: "code",
...
if (isError) return <p>خطأ: {message}</p>;`,
    ...
    {
      type: "paragraph",
      text: "إجمالي السلّة مشتقّ من مصفوفة items. useMemo يخزّن النتيجة ولا يُعيد حسابها إلا عند تغيّر items — مهمّ عندما تحوي السلّة عناصر كثيرة.",
    },
    {
      type: "code",
  ...
  [items]
    ...
    {
      type: "tip",
      text: "toFixed(2) يصيغ السعر إلى منزلتين عشريتين. بدونه ينتج حساب الفاصلة العائمة قيماً مثل $29.990000000003.",
    },
...
export default Products;`,
  ...
  lessonsEn: [
    ...
    "Reading the Cart with useSelector",
    "Updating the Cart with useDispatch",
    "Removing Items from Cart",
    "Creating the Favorites Slice",
    "Favorites: Context vs Redux",
    "Async Products with createAsyncThunk",
  ...
  contentEn: [
    ...
    {
      type: "paragraph",
      text: "This is the most complex project in the course. Plan carefully before coding — understand what state lives where and why.",
    },
    {
      type: "list",
      items: [
        "Browse products fetched from a real API (Fake Store API)",
        "Add products to a shopping cart with quantity tracking",
        "Mark products as favorites (toggle heart icon)",
        "Live cart total + async loading/error states managed by Redux",
      ],
    },
    {
      type: "code",
...
└── components/
    ...
    {
      type: "tip",
      text: "One slice = one concern. Products, cart, and favorites are separate slices so they can evolve independently.",
    },

    { type: "heading", text: "2. Creating the Products Page" },
    {
      type: "paragraph",
      text: "Products.jsx is the main page. It dispatches the async thunk on mount and reads the products list from the Redux store.",
    },
    {
      type: "code",
      code: `// pages/Products.jsx
...
function Products() {
  ...
  return (
    ...
    { type: "heading", text: "3. Product Card Component" },
    {
      type: "paragraph",
      text: "ProductCard displays one product and lets the user add it to the cart or toggle it as a favorite. It dispatches actions — it does NOT own any state.",
    },
    {
      type: "code",
      code: `// components/ProductCard.jsx
...
function ProductCard({ product }) {
  ...
  return (
    ...
    { type: "heading", text: "4. Fetching Products from API" },
    {
      type: "paragraph",
      text: "createAsyncThunk handles the async lifecycle automatically: pending → fulfilled → rejected. You write the async logic once and Redux generates the action types.",
    },
    {
      type: "code",
      code: `// store/productsSlice.js
...
export const fetchProducts = createAsyncThunk(
  ...
  async () => {
    ...
    {
      type: "tip",
      text: "The first argument to createAsyncThunk ('products/fetch') is the action type prefix. Redux Toolkit auto-generates products/fetch/pending, /fulfilled, /rejected.",
    },

    { type: "heading", text: "5. Creating the Cart Slice" },
    {
      type: "paragraph",
      text: "createSlice generates action creators and the reducer in one place. The initialState defines the exact shape of the cart state.",
    },
    {
      type: "code",
      code: `// store/cartSlice.js
...
export default cartSlice.reducer;`,
    ...
    {
      type: "tip",
      text: "Redux Toolkit uses Immer under the hood, so you CAN mutate state directly inside a reducer. It's translated to immutable updates automatically.",
    },

    { type: "heading", text: "6. Adding Products to Cart" },
    {
      type: "paragraph",
      text: "If the product is already in the cart, increment its quantity. Otherwise push a new entry with qty: 1. This is the most important cart logic.",
    },
    {
      type: "code",
      code: `addToCart: (state, action) => {
  ...
  if (found) {
    ...
    { type: "heading", text: "7. Reading the Cart with useSelector" },
    {
      type: "paragraph",
      text: "useSelector subscribes to the Redux store. Whenever the selected slice of state changes, the component re-renders automatically.",
    },
    {
      type: "code",
      code: `// In Cart.jsx or any component
...
function Cart() {
  ...
  return (
    ...
    { type: "heading", text: "8. Updating the Cart with useDispatch" },
    {
      type: "paragraph",
      text: "useDispatch gives you the store's dispatch function. Call it with an action creator to update state. Action creators are just functions that return an action object.",
    },
    {
      type: "code",
...
function SomeComponent({ product }) {
  ...
  return (
    ...
    { type: "heading", text: "9. Removing Items from Cart" },
    {
      type: "paragraph",
      text: "removeFromCart filters out the item by id. The payload is the product's id (a number), not the whole product object.",
    },
    {
      type: "code",
      code: `removeFromCart: (state, action) => {
  ...
  state.items = state.items.filter((i) => i.id !== action.payload);
    ...
    {
      type: "tip",
      text: "When dispatching: dispatch(removeFromCart(product.id)) — pass just the id, not the whole object. Keep payloads minimal.",
    },

    { type: "heading", text: "10. Creating the Favorites Slice" },
    {
      type: "paragraph",
      text: "Favorites are just an array of product ids. The toggleFavorite reducer removes the id if it's already there, otherwise adds it. Here we return a brand-new array — when a reducer returns a value, Redux Toolkit uses it as the next state.",
    },
    {
      type: "code",
      code: `// store/favoritesSlice.js
...
export default favoritesSlice.reducer;`,
    ...
    { type: "heading", text: "11. Favorites: Context vs Redux" },
    {
      type: "paragraph",
      text: "Context would work technically for favorites (it's simple data). But this app already uses Redux for products and cart — keeping all global state in one system is cleaner and avoids a mix of patterns.",
    },
    {
      type: "tip",
      text: "Rule: pick the simplest tool that fits, but be consistent. Adding Context on top of Redux in the same app creates two competing state systems that are harder to debug.",
    },

    { type: "heading", text: "12. Async Products with createAsyncThunk" },
    {
      type: "paragraph",
      text: "Wire the thunk's lifecycle (pending / fulfilled / rejected) into the slice using extraReducers. This is how Redux knows to update isLoading and items when the fetch settles.",
    },
    {
      type: "code",
      code: `// store/productsSlice.js
const productsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    { type: "heading", text: "13. Loading and Error UI" },
    {
      type: "paragraph",
      text: "Read isLoading and isError from the store and show the appropriate UI. Return early so the product grid only renders when data is ready.",
    },
    {
      type: "code",
      code: `const { items, isLoading, isError, message } =
  useSelector((s) => s.products);
...
return (
  <div className="grid">
    ...
    { type: "heading", text: "14. Cart Total with useMemo" },
    {
      type: "paragraph",
      text: "The cart total is derived from the items array. useMemo caches the result and only recalculates when items changes — important when the cart has many items.",
    },
    {
      type: "code",
...
function Cart() {
  ...
  return (
    ...
    {
      type: "tip",
      text: "toFixed(2) formats the price to two decimal places. Without it, floating-point arithmetic produces values like $29.990000000003.",
    },

    { type: "heading", text: "15. Final Refactor" },
    {
      type: "paragraph",
      text: "Audit the codebase for consistency and separation of concerns before calling it done.",
    },
    {
      type: "list",
      items: [
        "Each slice in its own file: products, cart, favorites — easy to locate and modify",
        "Async loading/error handled in the store via extraReducers — components stay clean",
        "Cart total derived with useMemo — not stored in state, never out of sync",
        "All global state in one Redux store — no mixing with Context or local state",
        "Components only dispatch and select — no business logic inside JSX",
      ],
    },
    ...
    {
      type: "paragraph",
      text: "The core of a real e-commerce frontend: products from an API, a working cart with quantities and a live total, and favorites — all powered by Redux Toolkit and async thunks. Add authentication, checkout, and a backend, and this is a production app.",
    },
    {
      type: "qa",
      question: "1. Why use createAsyncThunk instead of fetching in useEffect?",
      answer:
        "createAsyncThunk integrates with Redux: loading/error state lives in the store and is accessible from any component. useEffect keeps fetch logic local to one component and requires you to manage loading/error state manually.",
    },
    {
      type: "qa",
