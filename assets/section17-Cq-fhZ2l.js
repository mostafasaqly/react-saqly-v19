const t={id:17,title:"المشروع الأول: تطبيق إدارة المهام",level:"مشروع",lessons:["نظرة عامة على المشروع","إنشاء تخطيط التطبيق","إنشاء مكوّنات قابلة لإعادة الاستخدام","إنشاء نموذج بيانات المهمة","إضافة المهام","تحديث المهام","حذف المهام","تعليم المهام كمكتملة","فلترة المهام","البحث في المهام","حفظ البيانات في Local Storage","إضافة إجراءات النماذج","إضافة واجهة الانتظار","إضافة التحديثات التفاؤلية","إعادة الهيكلة النهائية","ملفات المشروع الكاملة"],intro:"مشروعنا الأول! تطبيق إدارة مهام تضيف وتعدّل وتكمل وتحذف وتفلتر وتبحث في المهام، وتحفظها بعد إعادة التحميل، وتستخدم إجراءات رياكت 19 وواجهة الانتظار والتحديثات التفاؤلية. أول قطعة في معرض أعمالك.",content:[{type:"heading",text:"1. نظرة عامة على المشروع"},{type:"paragraph",text:"قبل كتابة أي سطر، افهم ما الذي تبنيه. صورة واضحة للهدف النهائي تجعل كل خطوة مقصودة."},{type:"list",items:["إضافة مهمة، وتعليمها مكتملة/نشطة، وتعديلها، وحذفها","فلترة (الكل / النشطة / المكتملة) وبحث بالنص","حفظ في localStorage لتبقى المهام بعد إعادة التحميل","إجراءات النماذج + واجهة انتظار + إضافة تفاؤلية (مزايا رياكت 19)"]},{type:"code",code:`src/
├── App.jsx
├── components/
│   ├── TaskForm.jsx
│   ├── TaskList.jsx
...
└── utils/task.js`},{type:"tip",text:"الخطوة الأولى دائماً: ارسم شجرة المكوّنات على ورقة قبل فتح المحرّر. توفّر عليك ساعات."},{type:"heading",text:"2. إنشاء تخطيط التطبيق"},{type:"paragraph",text:"App.jsx هو «العقل». يملك قائمة المهام والفلتر النشط وكلمة البحث، ثم يمرّر البيانات ودوال المعالجة لأسفل. هذا نمط «رفع الحالة لأعلى» في العمل."},{type:"tip",text:"القاعدة: ضع الحالة في أعلى مكوّن يحتاجها، لا أعلى من ذلك. App.jsx هو المكان الصحيح هنا لأن كل الأبناء يحتاجون بيانات المهام."},{type:"heading",text:"3. إنشاء مكوّنات قابلة لإعادة الاستخدام"},{type:"paragraph",text:"نقسّم الواجهة لقطع صغيرة ذات مهمّة واحدة: TaskForm و TaskList و TaskItem و FilterBar و SearchBar. المكوّنات الصغيرة أسهل قراءةً وإعادة استخدام."},{type:"heading",text:"4. إنشاء نموذج بيانات المهمة"},{type:"paragraph",text:"قبل كتابة الميزات، حدّد شكل المهمة. نموذج واضح يمنع الأخطاء."},{type:"code",code:`export function createTask(text) {
  return {
...
);`},{type:"tip",text:"كلٌّ من الفلتر والبحث حالة مشتقّة — تُحسَب أثناء الرسم في App.jsx، ولا تُخزَّن كمصفوفة منفصلة أبداً."},{type:"heading",text:"11. حفظ البيانات في Local Storage"},{type:"paragraph",text:"نريد بقاء المهام بعد إعادة التحميل. خطّاف useLocalStorage يتولّى ذلك — فقط بدّل useState به."},{type:"code",code:'const [tasks, setTasks] = useLocalStorage("tasks", []);'},{type:"heading",text:"12. إضافة إجراءات النماذج (رياكت 19)"},{type:"paragraph",text:"رياكت 19 يسمح لخاصية action في النموذج بقبول دالة غير متزامنة. هذا يستبدل نمط onSubmit + e.preventDefault() اليدوي ويتكامل مع useFormStatus."},{type:"code",code:`async function handleAdd(formData) {
...
}`},{type:"tip",text:"يجب أن يكون SubmitButton ابناً لعنصر form — لأن useFormStatus يقرأ أقرب نموذج سابق له."},{type:"heading",text:"14. إضافة التحديثات التفاؤلية"},{type:"paragraph",text:"useOptimistic يعرض المهمة الجديدة فوراً بينما الإجراء غير المتزامن ما زال يعمل. إذا فشل الإجراء، يتراجع رياكت إلى الحالة الحقيقية تلقائياً."},{type:"code",code:`const [optimisticTasks, addOptimisticTask] = useOptimistic(
  tasks,
  (current, newTask) => [...current, { ...newTask, sending: true }]
);`},{type:"heading",text:"15. إعادة الهيكلة النهائية"},{type:"list",items:["شكل المهمة في نموذج واحد","مكوّنات صغيرة ذات مهمّة واحدة","حالة مشتقّة للفلتر والبحث","useLocalStorage و SubmitButton قابلان لإعادة الاستخدام"]},{type:"tip",text:"تحدّيات: زر «مسح المكتملة»، عدّاد «3 من 7 مكتملة»، تاريخ استحقاق، تعديل بالنقر المزدوج."},{type:"heading",text:"🎉 ما بنيته"},{type:"paragraph",text:"تطبيق إدارة مهام كامل ودائم يستخدم رياكت الأساسي مع إجراءات رياكت 19 وواجهة الانتظار والتحديثات التفاؤلية. كل نمط هنا — رفع الحالة، الحالة المشتقّة، الخطّافات المخصّصة، تركيب المكوّنات — يظهر في الأكواد الاحترافية يومياً. يستحق معرض الأعمال!"},{type:"qa",question:"1. لماذا يملك App.jsx كل الحالة بدلاً من كل مكوّن على حدة؟",answer:"لأن الفلتر والبحث والمهام كلها تؤثّر في نفس القائمة المرئية. امتلاكها في مكان واحد يبقي مصدراً واحداً للحقيقة ويتجنّب أخطاء المزامنة بين الأشقّاء."},{type:"qa",question:"2. ما الفرق بين التحديثات التفاؤلية وواجهة الانتظار؟",answer:"واجهة الانتظار تعطّل الزر أثناء الانتظار (صادقة لكنها تبدو بطيئة). التحديثات التفاؤلية تعرض النتيجة فوراً وتتراجع إن فشل شيء (تبدو سريعة لكنها تحتاج إجراءً غير متزامن حقيقياً)."},{type:"heading",text:"ملفات المشروع الكاملة"},{type:"paragraph",text:"فيما يلي الكود الكامل لكل ملف في المشروع. كل مقطع مُعنوَن باسم الملف الذي ينتمي إليه."},{type:"heading",text:"utils/task.js"},{type:"code",code:`// utils/task.js
export function createTask(text) {
  return {
    id: crypto.randomUUID(),
    text: text.trim(),
