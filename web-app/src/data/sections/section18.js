...
└── components/
    ...
    {
      type: "tip",
      text: "افصل الاهتمامات من اليوم الأول: services/ لنداءات الشبكة، hooks/ للمنطق القابل لإعادة الاستخدام، pages/ لComponents المسارات، components/ لقطع الواجهة.",
    },
  ...
  return (
    ...
    {
      type: "paragraph",
      text: "Hook useFetch المخصّص يغلّف منطق التحميل/الخطأ/البيانات، فيستطيع أي Component يحتاج بيانات استخدامه في سطر واحد.",
    },
    {
      type: "code",
...
export const URLS = { users: "/users", user: (id) => \`/users/\${id}\` };`,
    ...
    { type: "heading", text: "15. إعادة الهيكلة النهائية" },
    {
      ...
      items: [
        ...
        "Hook useFetch (Axios) قابل لإعادة الاستخدام",
    ...
    {
      type: "paragraph",
      text: "لوحة بيانات حقيقية بتوجيه وبحث وصفحات تفاصيل وهياكل تحميل ومعالجة أخطاء — جوهر عدد لا يُحصى من تطبيقات الإنتاج. كل نمط هنا (Hooks المخصّصة، طبقة الخدمات، حالة التنقّل + المعاملات كبديل) يُستخدم في أكواد React الاحترافية يومياً.",
    },
    {
      type: "qa",
      question: "1. لماذا يعيش useFetch في مجلد hooks/ بدلاً من داخل Component؟",
      answer:
        "لأن منطق التحميل/الخطأ/البيانات واحد لكل جلب — استخراجه في Hook يعني أن تكتبه مرة وتستخدمه في كل مكان. ويبقى Component مركّزاً على الرسم.",
    },
    {
      type: "qa",
