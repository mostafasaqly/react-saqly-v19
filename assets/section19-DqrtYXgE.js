const t={id:19,title:"المشروع الثالث: Store مصغّر بـ Redux",level:"مشروع",lessons:["نظرة عامة على المشروع","إنشاء صفحة المنتجات","Component بطاقة المنتج","جلب المنتجات من API","إنشاء Slice السلّة","إضافة منتجات للسلّة","قراءة السلّة بـ useSelector","تحديث السلّة بـ useDispatch","حذف عناصر من السلّة","إنشاء Slice المفضّلات","المفضّلات بContext أو Redux","منتجات غير متزامنة بـ createAsyncThunk","واجهة التحميل والخطأ","إجمالي السلّة بـ useMemo","إعادة الهيكلة النهائية","ملفات المشروع الكاملة"],intro:"مشروعنا الثالث والأكبر: Store صغير. يجلب المنتجات من API بـ Redux غير المتزامن، ويتيح إضافة عناصر للسلّة وقائمة مفضّلات، ويعرض إجمالي سلّة حيّ. يجمع Redux Toolkit والـ thunks وكل ما سبق.",content:[{type:"heading",text:"1. نظرة عامة على المشروع"},{type:"paragraph",text:"هذا أعقد مشروع في الكورس. خطّط بعناية قبل البرمجة — افهم أين تعيش كل حالة ولماذا."},{type:"list",items:["تصفّح منتجات مجلوبة من API حقيقي (Fake Store API)","إضافة منتجات للسلّة مع تتبّع الكمّيات","تعليم المنتجات كمفضّلة (تبديل أيقونة القلب)","إجمالي سلّة حيّ + حالات تحميل وخطأ يديرها Redux"]},{type:"code",code:`src/
├── store/
│   ├── store.js
...
└── components/
    ├── ProductCard.jsx
    └── Cart.jsx`},{type:"tip",text:"Slice واحدة = اهتمام واحد. المنتجات والسلّة والمفضّلات Slices منفصلة لتتطوّر كل منها باستقلال."},{type:"heading",text:"2. إنشاء صفحة المنتجات"},{type:"paragraph",text:"Products.jsx هي الصفحة الرئيسية. تُطلق الـ thunk غير المتزامن عند التركيب وتقرأ قائمة المنتجات من Store Redux."},{type:"code",code:`function Products() {
  const dispatch = useDispatch();
  const { items, isLoading } = useSelector((s) => s.products);
  useEffect(() => { dispatch(fetchProducts()); }, [dispatch]);
  // ...
}`},{type:"heading",text:"3. Component بطاقة المنتج"},{type:"paragraph",text:"ProductCard يعرض منتجاً واحداً ويتيح إضافته للسلّة أو تبديله كمفضّل. يُطلق Actions (dispatch) — ولا يملك أي حالة."},{type:"code",code:`function ProductCard({ product }) {
  const dispatch = useDispatch();
  return (
    <div className="card">
...
}`},{type:"heading",text:"4. جلب المنتجات من API"},{type:"paragraph",text:"createAsyncThunk يدير Lifecycle غير المتزامنة تلقائياً: pending ← fulfilled ← rejected. تكتب المنطق غير المتزامن مرة واحدة، ويولّد Redux أنواع Actions."},{type:"code",code:`export const fetchProducts = createAsyncThunk("products/fetch", async () => {
  const { data } = await axios.get("https://fakestoreapi.com/products");
  return data;
});`},{type:"tip",text:'أول وسيط لـ createAsyncThunk ("products/fetch") هو بادئة نوع Action. يولّد Redux Toolkit تلقائياً products/fetch/pending و /fulfilled و /rejected.'},{type:"heading",text:"5. إنشاء Slice السلّة"},{type:"paragraph",text:"createSlice يولّد مُنشئات Actions وReducer في مكان واحد. ويحدّد initialState شكل حالة السلّة بالضبط."},{type:"code",code:`const cartSlice = createSlice({
  name: "cart",
  initialState: { items: [] },
  reducers: { addToCart, removeFromCart, clearCart },
});`},{type:"tip",text:"Redux Toolkit يستخدم Immer تحت الغطاء، فيمكنك تعديل الحالة مباشرةً داخل Reducer — ويُترجَم إلى تحديثات غير قابلة للتغيير تلقائياً."},{type:"heading",text:"6. إضافة منتجات للسلّة"},{type:"paragraph",text:"إن كان المنتج موجوداً في السلّة، زِد كمّيته. وإلا أضف عنصراً جديداً بـ qty: 1. هذا أهم منطق في السلّة."},{type:"code",code:`addToCart: (state, action) => {
  const found = state.items.find((i) => i.id === action.payload.id);
  if (found) found.qty += 1;
  else state.items.push({ ...action.payload, qty: 1 });
}`},{type:"heading",text:"7. قراءة السلّة بـ useSelector"},{type:"paragraph",text:"useSelector يشترك في Store Redux. كلما تغيّرت Slice المختارة من الحالة، يُعاد رسم Component تلقائياً."},{type:"code",code:"const items = useSelector((state) => state.cart.items);"},{type:"heading",text:"8. تحديث السلّة بـ useDispatch"},{type:"paragraph",text:"useDispatch يعطيك دالة dispatch الخاصة بStore. ناديها بمُنشئ Action لتحديث الحالة. مُنشئات Actions مجرّد دوال تُرجع كائن Action."},{type:"code",code:`dispatch(addToCart(product));
...
}`},{type:"heading",text:"10. إنشاء Slice المفضّلات"},{type:"code",code:`toggleFavorite: (state, action) => {
  const id = action.payload;
  return state.includes(id) ? state.filter((x) => x !== id) : [...state, id];
}`},{type:"heading",text:"11. المفضّلات بContext أو Redux"},{type:"paragraph",text:"Context يصلح لأن المفضّلات بيانات بسيطة. لكن Redux أنسب هنا لأن التطبيق يستخدمه أصلاً للسلّة والمنتجات. إبقاء كل الحالة العامة في مكان واحد أنظف."},{type:"tip",text:"الدرس: نادراً ما يوجد جواب «صحيح» واحد. اختر الأداة التي تبقي تطبيقك متّسقاً."},{type:"heading",text:"12. منتجات غير متزامنة بـ createAsyncThunk"},{type:"paragraph",text:"اربط دورة حياة الـ thunk (pending / fulfilled / rejected) بSlice عبر extraReducers. هكذا يعرف Redux متى يحدّث isLoading و items عند انتهاء الجلب."},{type:"code",code:`.addCase(fetchProducts.fulfilled, (state, action) => {
  state.isLoading = false;
  state.items = action.payload;
})`},{type:"heading",text:"13. واجهة التحميل والخطأ"},{type:"paragraph",text:"اقرأ isLoading و isError من Store واعرض الواجهة المناسبة. ارجع مبكّراً لكي تُرسَم شبكة المنتجات فقط عند جاهزية البيانات."},{type:"code",code:`if (isLoading) return <p>جارٍ تحميل المنتجات...</p>;
if (isError) return <p>خطأ: {message}</p>;`},{type:"heading",text:"14. إجمالي السلّة بـ useMemo"},{type:"paragraph",text:"إجمالي السلّة مشتقّ من مصفوفة items. useMemo يخزّن النتيجة ولا يُعيد حسابها إلا عند تغيّر items — مهمّ عندما تحوي السلّة عناصر كثيرة."},{type:"code",code:`const total = useMemo(
  ...
  [items]
);`},{type:"tip",text:"toFixed(2) يصيغ السعر إلى منزلتين عشريتين. بدونه ينتج حساب الفاصلة العائمة قيماً مثل $29.990000000003."},{type:"heading",text:"15. إعادة الهيكلة النهائية"},{type:"list",items:["كل Slice في ملفها (products، cart، favorites)","التحميل/الخطأ غير المتزامن في Store عبر الـ thunks","إجمالي السلّة مشتقّ بـ useMemo","كل الحالة العامة في نظام واحد (Redux) للاتّساق"]},{type:"heading",text:"🎉 ما بنيته"},{type:"paragraph",text:"جوهر واجهة Store إلكتروني حقيقي: منتجات من API، وسلّة عاملة بكمّيات وإجمالي حيّ، ومفضّلات — كلها مدعومة بـ Redux Toolkit والـ thunks غير المتزامنة. أضِف مصادقة وإتمام شراء وخادماً، فيصير تطبيق إنتاج."},{type:"qa",question:"1. لماذا نستخدم createAsyncThunk بدلاً من الجلب داخل useEffect؟",answer:"‏createAsyncThunk يتكامل مع Redux: حالة التحميل/الخطأ تعيش في Store ويمكن الوصول إليها من أي Component. أما useEffect فيُبقي منطق الجلب محلياً في Component واحد ويتطلّب إدارة حالة التحميل/الخطأ يدوياً."},{type:"qa",question:"2. لماذا يُحسَب إجمالي السلّة بـ useMemo بدلاً من تخزينه في Slice cartTotal؟",answer:"لأن الإجمالي قابل للاشتقاق دائماً من items. تخزينه منفصلاً يخلق خطر خروجه عن المزامنة مع البيانات الحقيقية. Derived State (المحسوبة من مصدر الحقيقة) أفضل دائماً من الحالة المكرّرة."},{type:"heading",text:"ملفات المشروع الكاملة"},{type:"paragraph",text:"فيما يلي الكود الكامل لكل ملف في المشروع. كل مقطع مُعنوَن باسم الملف الذي ينتمي إليه."},{type:"heading",text:"store/productsSlice.js"},{type:"code",code:`// store/productsSlice.js
