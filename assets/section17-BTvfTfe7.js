const t={id:17,title:"المشروع الأول: تطبيق إدارة المهام",level:"مشروع",lessons:["نظرة عامة على المشروع","إنشاء تخطيط التطبيق","إنشاء Components قابلة لإعادة الاستخدام","إنشاء نموذج بيانات المهمة","إضافة المهام","تحديث المهام","حذف المهام","تعليم المهام كمكتملة","فلترة المهام","البحث في المهام","حفظ البيانات في Local Storage","إضافة Actions النماذج","إضافة واجهة الانتظار","إضافة التحديثات التفاؤلية","إعادة الهيكلة النهائية","ملفات المشروع الكاملة"],intro:"مشروعنا الأول! تطبيق إدارة مهام تضيف وتعدّل وتكمل وتحذف وتفلتر وتبحث في المهام، وتحفظها بعد إعادة التحميل، وتستخدم Actions React 19 وواجهة الانتظار والتحديثات التفاؤلية. أول قطعة في معرض أعمالك.",content:[{type:"heading",text:"1. نظرة عامة على المشروع"},{type:"paragraph",text:"قبل كتابة أي سطر، افهم ما الذي تبنيه. صورة واضحة للهدف النهائي تجعل كل خطوة مقصودة."},{type:"list",items:["إضافة مهمة، وتعليمها مكتملة/نشطة، وتعديلها، وحذفها","فلترة (الكل / النشطة / المكتملة) وبحث بالنص","حفظ في localStorage لتبقى المهام بعد إعادة التحميل","Actions النماذج + واجهة انتظار + إضافة تفاؤلية (مزايا React 19)"]},{type:"code",code:`src/
├── App.jsx
├── components/
│   ├── TaskForm.jsx
│   ├── TaskList.jsx
...
└── utils/task.js`},{type:"tip",text:"الخطوة الأولى دائماً: ارسم شجرة Components على ورقة قبل فتح المحرّر. توفّر عليك ساعات."},{type:"heading",text:"2. إنشاء تخطيط التطبيق"},{type:"paragraph",text:"App.jsx هو «العقل». يملك قائمة المهام والفلتر النشط وكلمة البحث، ثم يمرّر البيانات ودوال المعالجة لأسفل. هذا نمط «رفع الحالة لأعلى» في العمل."},{type:"tip",text:"القاعدة: ضع الحالة في أعلى Component يحتاجها، لا أعلى من ذلك. App.jsx هو المكان الصحيح هنا لأن كل الأبناء يحتاجون بيانات المهام."},{type:"heading",text:"3. إنشاء Components قابلة لإعادة الاستخدام"},{type:"paragraph",text:"نقسّم الواجهة لقطع صغيرة ذات مهمّة واحدة: TaskForm و TaskList و TaskItem و FilterBar و SearchBar. Components الصغيرة أسهل قراءةً وإعادة استخدام."},{type:"heading",text:"4. إنشاء نموذج بيانات المهمة"},{type:"paragraph",text:"قبل كتابة الميزات، حدّد شكل المهمة. نموذج واضح يمنع الأخطاء."},{type:"code",code:`export function createTask(text) {
  return {
...
}`},{type:"heading",text:"5. إضافة المهام"},{type:"paragraph",text:"أنشئ كائن مهمة جديد وأضفه لنهاية القائمة. أنتج دائماً مصفوفة جديدة — لا تعدّل الحالة مباشرةً أبداً."},{type:"code",code:`function addTask(text) {
  setTasks([...tasks, createTask(text)]);
}`},{type:"paragraph",text:"TaskForm ينادي onAdd(text) عند الإرسال. هو لا يملك قائمة المهام — يطلق حدثاً لأعلى فقط."},{type:"heading",text:"6. تحديث المهام"},{type:"paragraph",text:"ابحث عن المهمة بالـ id واستبدل حقل النص فقط. map() هي الأداة الصحيحة — تُرجع مصفوفة جديدة بعنصر واحد مُبدَّل."},{type:"code",code:`function editTask(id, newText) {
  setTasks(tasks.map((t) => (t.id === id ? { ...t, text: newText } : t)));
}`},{type:"tip",text:"النمط: { ...t, text: newText } يبقي كل الحقول الأخرى (done و createdAt) ويغيّر النص فقط."},{type:"heading",text:"7. حذف المهام"},{type:"paragraph",text:"filter() تُرجع كل مهمة لا يطابق id الخاص بها — أي تحذف المهمة المستهدفة فعلياً."},{type:"code",code:`function deleteTask(id) {
  setTasks(tasks.filter((t) => t.id !== id));
}`},{type:"heading",text:"8. تعليم المهام كمكتملة"},{type:"paragraph",text:"بدّل القيمة المنطقية done. map() مجدّداً — ابحث بالـ id، اقلب done، وأبقِ الباقي كما هو."},{type:"code",code:`function toggleTask(id) {
  setTasks(tasks.map((t) => (t.id === id ? { ...t, done: !t.done } : t)));
}`},{type:"heading",text:"9. فلترة المهام"},{type:"paragraph",text:"اشتقّ القائمة المرئية من مصفوفة المهام الكاملة والفلتر النشط. هذه حالة مشتقّة — لا تخزّن القائمة المفلترة بشكل منفصل."},{type:"code",code:`const byFilter = tasks.filter((task) => {
  if (filter === "active") return !task.done;
  if (filter === "completed") return task.done;
  return true;
});`},{type:"heading",text:"10. البحث في المهام"},{type:"paragraph",text:"ابنِ البحث فوق القائمة المفلترة بالفعل. خطوتان مشتقّتان، وقائمة مرئية نهائية واحدة."},{type:"code",code:`const visibleTasks = byFilter.filter((task) =>
  task.text.toLowerCase().includes(search.toLowerCase())
);`},{type:"tip",text:"كلٌّ من الفلتر والبحث حالة مشتقّة — تُحسَب أثناء الرسم في App.jsx، ولا تُخزَّن كمصفوفة منفصلة أبداً."},{type:"heading",text:"11. حفظ البيانات في Local Storage"},{type:"paragraph",text:"نريد بقاء المهام بعد إعادة التحميل. Hook useLocalStorage يتولّى ذلك — فقط بدّل useState به."},{type:"code",code:'const [tasks, setTasks] = useLocalStorage("tasks", []);'},{type:"heading",text:"12. إضافة Actions النماذج (React 19)"},{type:"paragraph",text:"React 19 يسمح لخاصية action في النموذج بقبول دالة غير متزامنة. هذا يستبدل نمط onSubmit + e.preventDefault() اليدوي ويتكامل مع useFormStatus."},{type:"code",code:`async function handleAdd(formData) {
  const text = formData.get("text");
  if (!text || !text.trim()) return;
  onAdd(text.trim());
}`},{type:"heading",text:"13. إضافة واجهة الانتظار"},{type:"paragraph",text:"useFormStatus (من react-dom) يعطيك حالة الانتظار لأقرب نموذج أب. ضع SubmitButton داخل النموذج ليتمكّن من قراءة تلك الحالة."},{type:"code",code:`import { useFormStatus } from "react-dom";
function SubmitButton() {
  const { pending } = useFormStatus();
  return <button disabled={pending}>{pending ? "جارٍ..." : "إضافة"}</button>;
}`},{type:"tip",text:"يجب أن يكون SubmitButton ابناً لعنصر form — لأن useFormStatus يقرأ أقرب نموذج سابق له."},{type:"heading",text:"14. إضافة التحديثات التفاؤلية"},{type:"paragraph",text:"useOptimistic يعرض المهمة الجديدة فوراً بينما Action غير المتزامن ما زال يعمل. إذا فشل Action، يتراجع React إلى الحالة الحقيقية تلقائياً."},{type:"code",code:`const [optimisticTasks, addOptimisticTask] = useOptimistic(
  tasks,
  (current, newTask) => [...current, { ...newTask, sending: true }]
);`},{type:"heading",text:"15. إعادة الهيكلة النهائية"},{type:"list",items:["شكل المهمة في نموذج واحد","Components صغيرة ذات مهمّة واحدة","حالة مشتقّة للفلتر والبحث","useLocalStorage و SubmitButton قابلان لإعادة الاستخدام"]},{type:"tip",text:"تحدّيات: زر «مسح المكتملة»، عدّاد «3 من 7 مكتملة»، تاريخ استحقاق، تعديل بالنقر المزدوج."},{type:"heading",text:"🎉 ما بنيته"},{type:"paragraph",text:"تطبيق إدارة مهام كامل ودائم يستخدم React الأساسي مع Actions React 19 وواجهة الانتظار والتحديثات التفاؤلية. كل نمط هنا — رفع الحالة، Derived State، Hooks المخصّصة، تركيب Components — يظهر في الأكواد الاحترافية يومياً. يستحق معرض الأعمال!"},{type:"qa",question:"1. لماذا يملك App.jsx كل الحالة بدلاً من كل Component على حدة؟",answer:"لأن الفلتر والبحث والمهام كلها تؤثّر في نفس القائمة المرئية. امتلاكها في مكان واحد يبقي مصدراً واحداً للحقيقة ويتجنّب أخطاء المزامنة بين الأشقّاء."},{type:"qa",question:"2. ما الفرق بين التحديثات التفاؤلية وواجهة الانتظار؟",answer:"واجهة الانتظار تعطّل الزر أثناء الانتظار (صادقة لكنها تبدو بطيئة). التحديثات التفاؤلية تعرض النتيجة فوراً وتتراجع إن فشل شيء (تبدو سريعة لكنها تحتاج Actionً غير متزامن حقيقياً)."},{type:"heading",text:"ملفات المشروع الكاملة"},{type:"paragraph",text:"فيما يلي الكود الكامل لكل ملف في المشروع. كل مقطع مُعنوَن باسم الملف الذي ينتمي إليه."},{type:"heading",text:"utils/task.js"},{type:"code",code:`// utils/task.js
export function createTask(text) {
  return {
    id: crypto.randomUUID(),
    text: text.trim(),
...
export default App;`}]};export{t as default};
